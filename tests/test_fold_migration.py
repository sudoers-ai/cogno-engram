...
async def _base_com_colisao(*, arestas_extra=()):
    ...
    ids = {}
    for rotulo in ("José", "Jose", "Rex", "Maria"):
        cur = await conn.execute(
            "INSERT INTO knowledge_nodes (scope, label, node_type) VALUES (%s,%s,%s) "
            "RETURNING id", ("t", rotulo, "PERSON"))
        ids[rotulo] = (await cur.fetchone())[0]
    # `José` tem 2 arestas, `Jose` tem 1 — o sobrevivente é o mais conectado
    padrao = [("José", "Rex", "OWNS_PET"), ("José", "Maria", "MARRIED_TO"),
              ("Jose", "Maria", "WORKS_WITH")]
    for s, t, r in list(padrao) + list(arestas_extra):
        await conn.execute(
...
async def test_the_naive_DELETE_would_lose_edges_and_the_merge_does_not():
    ...
    # (2) a ferramenta
    fusoes = await merge_fold_collisions(conn, dry_run=False)
    depois = await _arestas(conn)
    await conn.close()

    assert len(fusoes) == 1 and fusoes[0].nos_apagados == 1
    assert len(depois) == 3, f"a fusão perdeu arestas: {antes - depois}"
    assert ("José", "WORKS_WITH", "Maria") in depois, (
...
async def test_the_dry_run_numbers_match_what_apply_actually_does():
    ...
    conn, _ = await _base_com_colisao()
    previsto = (await merge_fold_collisions(conn, dry_run=True))[0]
    feito = (await merge_fold_collisions(conn, dry_run=False))[0]
    await conn.close()

    assert (previsto.arestas_repontadas, previsto.arestas_duplicadas_removidas,
            previsto.auto_arestas_removidas, previsto.nos_apagados) == \
           (feito.arestas_repontadas, feito.arestas_duplicadas_removidas,
            feito.auto_arestas_removidas, feito.nos_apagados)
...
async def test_an_edge_that_would_DUPLICATE_is_dropped_not_crashed_on():
    ...
    conn, _ = await _base_com_colisao(arestas_extra=[("Jose", "Rex", "OWNS_PET")])
    fusao = (await merge_fold_collisions(conn, dry_run=False))[0]
    arestas = await _arestas(conn)
    await conn.close()

    assert fusao.arestas_duplicadas_removidas == 1
    assert ("José", "OWNS_PET", "Rex") in arestas
    assert len([a for a in arestas if a[1] == "OWNS_PET"]) == 1, "ficou duplicada"
...
async def test_an_edge_BETWEEN_the_two_would_become_a_self_loop_and_is_dropped():
    ...
    conn, _ = await _base_com_colisao(arestas_extra=[("José", "Jose", "SAME_AS")])
    fusao = (await merge_fold_collisions(conn, dry_run=False))[0]
    arestas = await _arestas(conn)
    await conn.close()

    assert fusao.auto_arestas_removidas == 1
    assert not [a for a in arestas if a[0] == a[2]], f"ficou um laço: {arestas}"
...
async def test_the_report_names_the_survivor_and_the_losers():
    ...
    conn, _ = await _base_com_colisao()
    colisoes = await fold_collisions(conn)
    await conn.close()

    assert len(colisoes) == 1
    texto = str(colisoes[0])
    assert "José" in texto and "Jose" in texto and "arestas" in texto, texto
    assert colisoes[0].sobrevivente.label == "José", "o mais conectado tinha de sobreviver"
